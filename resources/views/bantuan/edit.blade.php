<x-app-layout>
  ...
  <div class="p-4 bg-white rounded-lg shadow-xs">
    <h1 class="text-2xl font-semibold text-gray-800">Edit Bantuan</h1>

    <div class="overflow-hidden mb-8 w-full rounded-lg shadow-xs">
      <div class="overflow-x-auto w-full">
        <form action="{{ route('bantuan.update', $bantuan->id) }}" method="POST" autocomplete="off">
          @csrf
          @method('PUT')
          ...
          <div class="mt-4">
            ...
            <select id="jenis_bantuan" name="jenis_bantuan"
              ...
              @foreach ($jenisBantuan as $jenis)
                <option value="{{ $jenis }}"
                  {{ old('jenis_bantuan', $bantuan->jenis_bantuan) == $jenis ? 'selected' : '' }}>
                  {{ $jenis }}
                </option>
              @endforeach
            </select>
          ...
          <div class="mt-4">
            ...
            <textarea name="keterangan" id="keterangan"
              class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-primary-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50 focus-within:text-primary-600"
              required>{{ old('keterangan', $bantuan->keterangan) }}</textarea>
